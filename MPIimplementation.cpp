...
vector<vector<Edge>> importFromCSV(const string &filename, int &numVertices)
{
    ...
    for (const auto &[src, dst, weight] : edges)
    {
        ...
        graph[dst].emplace_back(src, weight); // For undirected graph
    }
...
vector<vector<Edge>> readGraph(const string &filename, int &numVertices)
{
    ...
    {
        return importFromCSV(filename, numVertices);
    }
                     ...
                     vector<vector<Edge>> &localGraph, int &localStart, int &localCount)
{
    ...
    if (rank == 0)
    {
        ...
        for (int r = 1; r < size; ++r)
        {
            ...
            {
                int edgeCount = fullGraph[v].size();
                      ...
                      vector<long long> &localDist, vector<int> &localParent)
{
    ...
    while (!done)
    {
        ...
        if (rank == ownerRank)
        {
            ...
            if (localVertex >= 1 && localVertex <= localCount)
            {
                for (const Edge &edge : localGraph[localVertex])
                {
                    ...
                    if (neighborOwner == rank)
                    {
                        ...
                        if (altDist < localDist[localNeighbor])
                        {
                         ...
                         int u, int v, int rank, int size, int localStart, int localCount)
{
    ...
    if (v >= localStart && v < localStart + localCount)
    {
        ...
        if (localV >= 0 && localV < (int)graph.size())
        {
    ...
    {
        int localU = u - localStart + 1;
        if (localU >= 0 && localU < (int)dist.size() && parent[localU] == v)
            needUpdate = true;
    }
    if (v >= localStart && v < localStart + localCount)
    {
        int localV = v - localStart + 1;
        if (localV >= 0 && localV < (int)dist.size() && parent[localV] == u)
            needUpdate = true;
    }
    ...
    if (needUpdate)
    {
        ...
        {
            int localU = u - localStart + 1;
            if (localU >= 0 && localU < (int)dist.size())
                pq.push({dist[localU], u});
        }
        if (v >= localStart && v < localStart + localCount)
        {
            int localV = v - localStart + 1;
            if (localV >= 0 && localV < (int)dist.size())
        ...
        while (!pq.empty())
        {
            ...
            if (currentVertex >= localStart && currentVertex < localStart + localCount)
            {
                ...
                if (localVertex >= 0 && localVertex < (int)graph.size())
                {
                    for (const Edge &edge : graph[localVertex])
                    {
                        int neighbor = edge.targetVertex;
                        long long newDist = currentDist + edge.weight;

                        int neighborOwner = getOwnerRank(neighbor, dist.size() - 1, size);
                        if (neighborOwner == rank)
                        {
                            int localNeighbor = neighbor - localStart + 1;
                            if (localNeighbor >= 0 && localNeighbor < (int)dist.size() && newDist < dist[localNeighbor])
                            {
...
void insertEdgeAndUpdate(vector<vector<Edge>> &graph, vector<long long> &dist, vector<int> &parent,
                         int u, int v, long long weight, int rank, int size, int localStart, int localCount)
{
    ...
    // Check if the new edge can improve any shortest paths
    priority_queue<pair<long long, int>, vector<pair<long long, int>>, Compare> pq;
...
int main(int argc, char **argv)
{
    ...
    vector<long long> globalDist, localDist;
    vector<int> globalParent, localParent;
    ...
    if (rank == 0)
    {
        globalDist.resize(numVertices + 1, INF);
        globalParent.resize(numVertices + 1, -1);
    }
    ...
    if (rank == 0)
    {
        ...
        exportToCSV(globalDist, "initial_distances_MPI.csv", numVertices);

        // Perform edge deletion
        auto start_deletion = chrono::high_resolution_clock::now();
        deleteEdgeAndUpdate(localGraph, globalDist, globalParent, 16395, 11, rank, size, localStart, localCount);
        ...
        exportToCSV(globalDist, "after_deletion_mpi.csv", numVertices);

        // Perform edge insertion
        auto start_insertion = chrono::high_resolution_clock::now();
        insertEdgeAndUpdate(localGraph, globalDist, globalParent, 16398, 12, 67000, rank, size, localStart, localCount);
        ...
        exportToCSV(globalDist, "Mpiafterinsertion.csv", numVertices);
    }

    // Broadcast updated distances to all processes
    vector<long long> tempDist(numVertices + 1);
    vector<int> tempParent(numVertices + 1);
    if (rank == 0)
    {
        tempDist = globalDist;
        tempParent = globalParent;
    }
    MPI_Bcast(tempDist.data(), numVertices + 1, MPI_LONG_LONG, 0, MPI_COMM_WORLD);
    MPI_Bcast(tempParent.data(), numVertices + 1, MPI_INT, 0, MPI_COMM_WORLD);

    // Update local distances and parents
    for (int i = localStart; i < localStart + localCount; ++i)
    {
        int localI = i - localStart + 1;
        if (localI >= 0 && localI < (int)localDist.size())
        {
            localDist[localI] = tempDist[i];
            localParent[localI] = tempParent[i];
        }
    }
